...
The project follows the "PSR-4":https://www.php-fig.org/psr/psr-4/ and "PSR-12":https://www.php-fig.org/psr/psr-12/ standards. You can use PHP_CodeSniffer to make sure your additions follow them too:
...
The project uses "PHPunit":https://phpunit.de for running its unit tests and "PHPStan":https://github.com/phpstan/phpstan for static analysis. As much of the codebase should be covered with tests as possible. When adding new features, also add tests for them. If you find a bug, create a test that can replicate the said bug and, if possible, fix it so that the tests pass. To run tests, simply run:

bc. $ composer test

Running tests requires PHP >=7.2.0 and PCRE with PCRE_UTF8 support. Generating optional code coverage reports requires "Xdebug":https://xdebug.org.
