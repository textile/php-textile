(function () {
    ...
    require(['jquery'], function ($) {
        var toggle = $('<div class="menu-toggle"><a href="#"><i class="fa fa-bars"></i></a></div>'),
            nav = $('nav'),
            body = $('body');
    ...
    require(['jquery'], function ($) {
        $('.selectable, pre').on('click', function (e) {
            ...
            var copy,
                ...
                selection,
                $this = $(this),
                region = $this.find('.selectable-region, code');
    ...
    require(['jquery'], function ($) {
        var form = $('form.async'),
            htmlview = $('<div class="view html" />'),
            preview = $('<div class="view preview" />');
            ...
            $.ajax($this.attr('action'), {
                ...
                .done(function (data) {
                    ...
                    if ($.type(data.output.restricted) === 'string') {
                        $this.append($('<div class="wrapper" />').html(
                            htmlview.html(
                                $('<pre class="prettyprint language-html" />')
                                    .html($('<code />').text(data.output.restricted))
                            )
                                .prepend('<h2>HTML source</h2>'))
                                .append(
                                    preview
                                        .html(data.output.restricted)
