...
Function     : Adds syntax highlighted code block for PHP.
Matches block: bcphp.
...
function _textile_bcphp_block_handler( $textile, $tag, $att, $atts, $ext, $cite, $o1, $o2, $content, $c2, $c1, $eat )
{
	if( $tag === 'bcphp' ) 
	{
		...
		$c1 = '</p>';
		$o2 = $c2 ='';
		$content = '<?php'.$content.' ?>'; // PHP tags rein damit hilghlighter tut
		$content = highlight_string($content, TRUE);
		$content = preg_replace('/&lt;\?php/', '', $content, 1); // opening PHP tag raus
		$content = preg_replace('/\?&gt;(?!.*\?&gt;)/', '', $content); // closing PHP tag raus
